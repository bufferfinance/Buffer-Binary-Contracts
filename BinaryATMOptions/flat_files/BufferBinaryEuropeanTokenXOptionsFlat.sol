    ...
    ReentrancyGuard
{
    ...
    event Mint(uint256 tokenId, address account, Tier tier);

    address public admin = 0xC8dA7b5C0d15db38ca9d24C94188F2ab0f4AC081;

    constructor() ERC721("Buffer Prime", "pBFR") {}
...
// File: BufferBinaryEuropeanTokenXOptions.sol
 ...
 */
contract BufferBinaryEuropeanTokenXOptions is
    IBufferOptions,
    Ownable,
    ReentrancyGuard,
    BufferNFTCore
{
    ...
    function create(
        ...
        uint256 period = expiration - block.timestamp;
        optionID = _quickCreate(
            amount,
            strike,
            period,
            isYes,
    ...
    /**
     * @notice Creates a new option by specifying the period instead
     * @param amount Option amount in tokenX
     * @param referrer Referrer address
     * @param metadata Option metadata
     * @return optionID Created option's ID
     */
    function _quickCreate(
        uint256 amount,
        uint256 strike,
        uint256 period,
        bool isYes,
        ...
        // if the period is lower than 24 hours then assert that the option be an ATM one and deduct a small fee
        if ((period) < 24 hours) {
            require(strike == 0, "O22");
        ...
        )
    {
        ...
        premium = FeeCalculator.feesTokenXBinary(
            period,
            amount,
            strike,
            isYes,
